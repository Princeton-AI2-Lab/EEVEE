...
class DataProcessor:
    """Processor for the FinLoRA FiNER tagging task."""

    def __init__(self, task_name: str = "finer"):
        self.task_name = task_name
        if self.task_name != "finer":
            raise ValueError(f"Unknown FiNER task: {self.task_name}")
    ...
    def process_task_data(self, raw_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for idx, item in enumerate(raw_data):
            ...
            if not prompt_text or not target:
                raise ValueError(f"FiNER item missing prompt/target: idx={idx}")
